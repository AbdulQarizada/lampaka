...
<div class="row">
    <div class="col-12">
        ...
        <form class="needs-validation" action="{{route('ExportExpense')}}" method="POST"   enctype="multipart/form-data" id="ExportForm" novalidate>
            @csrf
...
<script>
    ...
    (async () => {
        const YealyExpensesConst = await fetch('{{route('YealyExpense_ColumnChart', ['data' => $PageInfo])}}').then(response => response.json());
